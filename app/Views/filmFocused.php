      ...
      <div class="focusedDivDescription">
        ...
        <a href="<?= base_url('liker?id_film=' . $film['id_film']) ?>">
            <svg class="loveIcon" width="37" height="34" viewBox="0 0 37 34" fill="none" xmlns="http://www.w3.org/2000/svg">
                <?php
                // Utilise le modèle LikesModel pour vérifier si le film est liké par l'utilisateur
                $likeModel = new \App\Models\LikesModel();
                $session = session();
                $id_user = $session->get('user')['id_user'];

                $isLiked = $likeModel->where(['id_film' => $film['id_film'], 'id_user' => $id_user])->countAllResults() > 0;
                ?>
                <?php if ($isLiked): ?> // Si l'utilisateur a déjà liké on affiche un coeur plein
